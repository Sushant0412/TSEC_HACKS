...
const BASE_URL =
  "https://newsdata.io/api/1/latest?apikey=pub_6698314d9058511cfb117c8dbaaf930626db7&q=court&country=in&language=en";

export const getLegalNewsController = async (req, res) => {
  try {
    const response = await axios.get(BASE_URL);
    console.log(response.data.results[0]); // Log to check the structure
    res.status(200).json(response.data); // Send the full response back to frontend
  } catch (error) {
    console.error("Error fetching legal news:", error.message);
