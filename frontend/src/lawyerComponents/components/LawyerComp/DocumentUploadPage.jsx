...
const DocumentUpload = () => {
    ...
    if (selectedClient && files.length > 0) {
      ...
      try {
        const response = await axios.post(
          "http://localhost:3000/lawyer/uploadDocument",
          formData,
          {
            headers: {
  ...
  const uploadedDocumentsList = uploadedDocuments.filter(
    (doc) => doc.clientId === selectedClient
  );
    ...
    <div className="p-6">
      ...
      <div className="text-left mb-8">
        ...
        <p className="text-lg text-gray-600 mt-2">
          Manage client documents efficiently for their bail application.
        </p>
      </div>
            ...
            <ul className="space-y-4">
              {missingDocuments.length > 0 ? (
                missingDocuments.map((doc, index) => (
                  <li
                    key={index}
                    className="bg-red-50 border border-red-200 text-red-600 rounded-lg shadow-sm p-4 flex items-center"
                  >
                    <FaExclamationCircle className="text-red-500 mr-2" />
                    {doc}
                  </li>
            ...
            <ul className="space-y-4">
              {uploadedDocumentsList.length > 0 ? (
                uploadedDocumentsList.map((doc) => (
                  <li
                    key={doc.id}
                    className="bg-green-50 border border-green-200 text-green-600 rounded-lg shadow-sm p-4 flex items-center"
                  >
                    <FaCheckCircle className="text-green-500 mr-2" />
                    <div>
