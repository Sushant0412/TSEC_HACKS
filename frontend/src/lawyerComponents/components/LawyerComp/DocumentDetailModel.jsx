    ...
    const fetchDocuments = async () => {
      try {
        const response = await axios.post(
          "http://localhost:3000/prisoner/getDocument",
          { prisonerId: "66c9913f6cdba09d94e14405" }
        );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-3xl relative max-h-[80vh] overflow-auto">
        ...
        <h2 className="text-2xl font-bold mb-4">
          {client.data.name} - {client.data.case_id}
        </h2>
              ...
              {Object.entries(documentLinks).map(([key, url]) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {key === "chargesComplaints" && (
                      <IdentificationIcon className="w-6 h-6 text-gray-500" />
                    )}
                    {key === "characterCertificates" && (
                      <UserCircleIcon className="w-6 h-6 text-gray-500" />
                    )}
                    {key === "legalNotices" && (
                      <ClipboardDocumentIcon className="w-6 h-6 text-gray-500" />
                    )}
                    {key === "statementsByAccused" && (
                      <DocumentTextIcon className="w-6 h-6 text-gray-500" />
                    )}
                    {key === "affidavits" && (
                      <IdentificationIcon className="w-6 h-6 text-gray-500" />
              ...
              <ul>
                {documents.map((doc) => (
                  <li key={doc._id}>
                    ...
                    <p>
                      <strong>File Path:</strong> {doc.filePath}
                    </p>
                    <p>
