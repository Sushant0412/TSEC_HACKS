  ...
  useEffect(() => {
    if (daysRemaining > 0) {
      const interval = setInterval(() => {
        ...
        setTotalDaysElapsed((prevElapsed) =>
          Math.min(prevElapsed + 1, totalDays)
        );
        ...
        <div className="mt-4">
            ...
            <input
              ...
              style={{
                background: `linear-gradient(to right, #0000FF 0%, #0000FF ${
                  (calculateWeeks(totalDaysElapsed) /
          ...
          <div className="text-center">
            <p className="text-lg text-gray-700 mb-2">
              {daysRemaining > 0
                ? `${daysRemaining} days remaining | ${Math.floor(
                    daysRemaining / 30
                  )} months remaining`
                : "Eligible for release"}
            </p>
            <p className="text-sm text-gray-500">
              {totalDaysElapsed} days elapsed |{" "}
              {calculateWeeks(totalDaysElapsed)} weeks elapsed
            </p>
          </div>
        </div>
      </div>
